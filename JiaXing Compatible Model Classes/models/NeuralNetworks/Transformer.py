...
class Transformer(TorchToSklearn_Model):
    ...
    class MLPPerFeature(nn.Module):
        ...
        def forward(self, X):
            ...
            mlp_output = torch.stack([self.shared_mlp(X[:, i:i+1]) for i in range(X.size(1))], dim=1)
            return mlp_output
        ...
        def __init__(self, CFG, hidden_dim, nhead, dim_feedforward, dropout, num_transformer_layers):
            ...
            self.transformer = nn.TransformerEncoder(
                encoder_layer=nn.TransformerEncoderLayer(
                    d_model=hidden_dim, 
                    nhead=nhead,
                    dim_feedforward=dim_feedforward, # usually 4x hidden_dim, but we set to be tune-able
                    dropout=dropout
                ),
    ...
    class Model(nn.Module):
        def __init__(self, CFG):
            ...
            self.mlp_per_feature = Transformer.MLPPerFeature(CFG, CFG.hidden_dim, CFG.dropout, CFG.batchnorm)

            # Transformer block
            self.transformer_block = Transformer.TransformerBlock(CFG, CFG.hidden_dim, CFG.nhead, CFG.dim_feedforward, CFG.dropout, CFG.num_transformer_layers)

            # MLP layer for final output
            if self.CFG.use_cls: # just take the 0th vector of the last layer (cls) in transformer_output
                self.out_mlp = LinearLayer(self.CFG, self.CFG.hidden_dim, self.CFG.output_dim)
            else: # concatenate the output from all layers in transformer_output
                self.out_mlp = LinearLayer(self.CFG, self.CFG.input_dim * self.CFG.hidden_dim, self.CFG.output_dim) 
        ...
        def forward(self, X):
            ...
            if self.CFG.use_cls:
                ...
                mlp_output = torch.cat([torch.zeros(X.size(0), 1, self.CFG.hidden_dim).to(X.device), mlp_output], dim=1)
    ...
    def __init__(self, 
        ...
        class CFG: 
            def __init__(self, input_dim, output_dim, num_transformer_layers, hidden_dim, dim_feedforward, nhead, use_cls, dropout, mode, batch_size, epochs, lr, random_state, grad_clip, batchnorm, loss, TabularDataFactory, TabularDataset, rootpath, name):
        ...
        self.CFG = CFG(input_dim, output_dim, num_transformer_layers, hidden_dim, dim_feedforward, nhead, use_cls, dropout, mode, batch_size, epochs, lr, random_state, grad_clip, batchnorm, loss, TabularDataFactory, TabularDataset, rootpath, name)
