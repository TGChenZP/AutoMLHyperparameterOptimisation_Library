...
class TorchToSklearn_Model(object):
    ...
    def __init__(self, CFG, name='Model'):
        ...
        assert self.CFG.mode in ['Classification', 'Regression'], 'mode must be either Classification or Regression'
    ...
    def fit(self, train_x, train_y):
        ...
        if self.CFG.mode == 'Classification': # if classification turn labels into e.g. 0 1 2 3 so data factory can turn into probability vectors
        ...
        x_list, y_list = self.CFG.TabularDataFactory(train_x, train_y, self.CFG.mode) # might want to do batch number as well
    ...
    def predict(self, val_x):
        ...
        with torch.no_grad():
            ...
            x_list = self.CFG.TabularDataFactory(val_x, mode=self.CFG.mode) # create the instances
            tabular_dataset = self.CFG.TabularDataset(x_list)

            val_dataloader = DataLoader(tabular_dataset, batch_size=self.CFG.batch_size, shuffle=False)
            ...
            for batch in val_dataloader:
                ...
                predicted_labels = torch.argmax(pred, dim=1) if self.CFG.mode == 'Classification' else pred.squeeze(1) # if regression, squeeze the dimension, if classification, argmax.
    ...
    def predict_proba(self, val_x):
        ...
        with torch.no_grad():
            ...
            for batch in val_dataloader:
                x_batch = batch.to(self.device)
                pred_probs = self.model(x_batch).softmax(dim=1) if self.CFG.mode == 'Classification' else self.model(x_batch).squeeze(1).sigmoid()
